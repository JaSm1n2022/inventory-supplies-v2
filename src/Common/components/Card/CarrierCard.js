...
function CarrierCard(props) {
  const { logo, shipmentNbr, carrierSelected,carrierInfo, currentItem, autoCompleteInputHander, onChangeInputHandler, carrierList, handleAssignCarrier, handleCarrierClose } = props;
      ...
      <CardContent>
        <div style={{ display: 'inline-flex' }}>
          ...
          <div style={{ width: '150px' }}>
            <Button size="medium" variant="contained" color="primary" onClick={() => handleAssignCarrier(currentItem)}>
            {carrierInfo && carrierInfo.name && carrierInfo.name !== '-' ? 'Reassign' : 'Assign'}
            </Button>
            &nbsp;
            <Button size="medium" variant="contained" color="default" onClick={() => handleCarrierClose()}>
              Cancel
            </Button>
          </div>
        </div>
...
export default withRouter(CarrierCard);