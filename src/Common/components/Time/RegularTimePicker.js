...
export default function RegularTimePicker(props) {
   const { id, source, onChange, variant, label, isError, type, value, errorMsg, readonly, name, placeholder, border, background, disabled, size, height, noDefault } = props;
   let colorBkg = background || '';
   colorBkg = disabled ? '#e9ecef' : colorBkg;
  //  let topLabel = label || '';
   let topLabel = noDefault && !value ? '' : (label || '');
